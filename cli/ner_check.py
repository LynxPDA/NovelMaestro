...
глоссария и релевантные фрагменты книги, где термин встречается.
...
def _batch_user_msg(title, batch, prompt_tpl, fields):
    ...
    (types-проход — с префиксом в системном сообщении).
    Используется и в предпросмотре."""
    body = glossary_body(batch, fields)
    tpl = prompt_tpl
    if title != "Весь глоссарий":
        # инструкция типового этапа — в системное сообщение (правила в
        # system, данные в user); промпт без <system> — префикс в начало
        if tpl.lstrip().startswith("<system>"):
            tpl = tpl.replace(
                "<system>", "<system>\n" + TYPES_STAGE_PREFIX.strip(), 1)
        else:
            tpl = TYPES_STAGE_PREFIX + tpl
