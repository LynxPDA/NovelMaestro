...
- **Запуски**: все стадии пайплайна, **«Простой режим / Экспертный»**
  ...
  **активного** запуска проекта (badge/бар/переход/стоп), лог
  (текстовое окно + SSE) — пока есть запуск (завершённый тоже остаётся
  видимым с итогами и таблицей глав; завершённый запуск по URL
  `…/{jobId}` открывает его лог). Таблица конвейера — события текущего
  запуска + фактическое состояние артефактов глав (статус проекта),
  так что галочки видны и для ранее переведённых глав. История логов —
  во вкладке «Логи» проекта. Метаданные всех запусков — `jobs.json`,
