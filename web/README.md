...
одна сборка. Финальные файлы сборки пишутся в КОРЕНЬ проекта
(`--out-dir`) с названием проекта в имени:
`{Имя}_{начало}_{конец}_txt.txt` / `.epub` / `.fb2`; рабочие файлы
(titles) — в `tmp/`, промежуточный txt для epub/fb2 не создаётся
(книга собирается из памяти).
TXT (Rulate)
