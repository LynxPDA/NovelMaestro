...
Статус: **работает** (v1.0 M1–M8 + v2 W1–W9 + раунд 4: web-first).
...
- **Проекты (hub)**: разделы ACTIVE/HOLD/DONE (динамические, раунд 22:
  ...
  (с подтверждением), реестр стадий `web/stages.py::STAGE_SPECS`
...
- **Редактор** (W8): CodeMirror 6 (вендор в `static/vendor/`, офлайн):
  раунд 23: вкладка «Редактор» — главы проекта (артефакты по маскам:
  ...
  справа полировка > редактура > перевод (по доступности); сохранение через
...
- **NER** (W5): таблица глоссария — инлайн-редактирование строк,
  ...
  добавление/удаление, поиск слева («Поиск:» + выбор полей), справа —
  «Показывать:» кнопки типов и столбцов; выбор столбцов/полей/типов —
  единая модалка «Все / набор» (всё снять нельзя — остаётся первый
  пункт, «Все» сверху); счётчик терминов — в шапке под «Запуски»;
  review-флоу:
...
- **Запуски** (раунд 20): все стадии пайплайна, динамические формы,
  live-лог + SSE, прогрессбар LLM-стадий (раунд 19: события
...
- **Настройки запусков** (R9): поля форм предзаполняются из `.env`
  ...
  не сохраняется (только предзаполнение формы; раунд 12: профили убраны).
...
- Прогресс (раунд 19): `payload().progress` и `_serialize` (jobs.json)
