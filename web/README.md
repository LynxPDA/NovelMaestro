...
- **Промпты конвейера**: форма pipeline — единый «Общий промпт-файл»
  ...
  `<pass1>/<pass2>`). Файл без тегов = промпт стадии целиком (легальный
  режим). Автоподхват — только реально существующие файлы: удалённый
...
- **Перевод с расширенным контекстом** (действие 9, малоресурсные
  ...
  «Входной JSON (ner.json)» — выбор файла глоссария в корне проекта
  (пусто = ner.json; только экспертный режим).
