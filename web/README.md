...
- **Перевод с расширенным контекстом** (действия 9/10, малоресурсные
  языки): «Перевод с расширенным контекстом» (стадия 1) и «Полный цикл
  с расширенным контекстом» (1,2,3) — словарь/правила/примеры из
  `source/` через «Файлы»:
  `dict.json` — словарь в формате ner.json (term/translation/type?/
  aliases?/notes?), ищется как глоссарий по чанку + source-сторонам
  выбранных примеров → `{dict_block}`;
  `rules.txt|md` — справочник языка, целиком в `{rules_block}`
  (потолок `rules_budget` СИМВОЛЫ);
  `examples.json` — пары `{original_text, translated_text}` (алиасы
  source/target; совместим с `translated_trace.json`), релевантные
  чанку пары — few-shot `{fewshot_block}` (containment n-грамм, топ-K
  со штрафом за пересечение, порог `fewshot_threshold` — лучше без
  примеров, чем с шумными).
  Плейсхолдеры заменяются во всех трёх стадиях (нет файла — «(нет)»);
