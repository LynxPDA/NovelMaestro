...
| POST | `/api/stages/batch_replace/preview` | batch_replace: предпросмотр замен в одной главе — `{project, type, chapter, replacements}`; правила — тем же путём, что реальный запуск (`cli.batch_replace.parse_replace_lines`/`apply_rules_segments`); ответ: `segments` `keep/del/ins` итогового текста + `stats` по правилам; файлы не изменяются |
