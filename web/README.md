...
python3 web/main.py             # 127.0.0.1:8756, без токена (локально)
python3 web/main.py --host 0.0.0.0 --auth   # доступ с LAN + токен
```

По умолчанию (W1) сервер слушает `127.0.0.1` — только этот компьютер
(`http://127.0.0.1:8756`), аутентификация **выключена** (локальный доступ
безопасен). Для доступа с других машин локальной сети — `--host 0.0.0.0`
(`http://<LAN-IP>:8756`) и **обязательно** включите токен: `--auth` или
`WEB_AUTH=1` — тогда вход по токену (`--token` > `.web_secret`,
автогенерация, chmod 600), а значения .env маскируются `••••`.
...
Порт занят (другая копия сервера) — `main.py` автоматически берёт
следующий свободный (`port+1 … port+100`) и пишет фактический адрес в
...
- **Проверка** (W7): отчёты translate_check (`logs/check_*.txt`) — таблица
  ...
  таб с поиском термина); неприменённые правки с причиной пропуска
  видны в карточке (поле `note`: цитата не найдена в книге / найдена
  в нескольких главах — не однозначно / файл главы не найден);
  пропущенные правки с уникальной цитатой автоматически
  перепривязываются к реальной главе (дрейф атрибуции LLM ±1: цитата
  ищется по всей книге, применение — только при ровно одном
  совпадении ровно в одной главе); JSON-редактор —
  прежний режим. Применение
  запускается как фоновая задача и уведомляет о завершении toast-ом
  (✅/❌/⏹); чекбокс «не создавать .bak» (по умолчанию бэкапы создаются).
- **Конфиг** (W6, раунд 8): системный `.env` — WEB_ENV_FILE (в Docker —
  `projects/.env` в томе, иначе корневой `.env` репо; `scope=global`),
  правится на странице «Настройки»; там же прозрачность слоёв —
  `sources` (ключ перекрыт os.environ — правка не применится) и
  `env_extra` (ключи окружения вне файла); проектный `pdir/.env` —
  изоляция проекта, приоритетнее системного; значения видимы без auth,
  маскируются при `--auth`; внешний вид (тема/кегль) — вкладка
  «Настройки», localStorage браузера (не .env);
...
- **Запуски**: все стадии пайплайна, **«Простой режим / Экспертный»**
  ...
  **активного** запуска проекта (badge/бар/переход/стоп), лог
  ...
  вкладке «Логи» проекта). Таблица конвейера — события текущего
  запуска + фактическое состояние артефактов глав (статус проекта),
  так что галочки видны и для ранее переведённых глав. Метаданные всех
...
- **Дашборд**: «Последние запуски» — до 20 записей
  ...
  «последних 20».
- **Промпты конвейера**: форма pipeline — единый «Общий промпт-файл»
  с тегами `<translate>/<redact>/<polish>` (режим промптов и отдельные
  файлы на стадию убраны). Пусто = auto: первый кандидат с тегами из
  `prompts/` (`pipeline_prompt.txt` → `prompts.txt` →
  `translate_book_prompt.txt`), иначе стадии используют встроенные
  промпты. Если в общем файле нет тега стадии — предупреждение в логе
  запуска, стадия уходит на встроенный промпт (то же в translate_book.py,
  ner.py `<prompt_pass1>/<prompt_pass2>`, translate_check_llm.py
  `<pass1>/<pass2>`, ner_check.py `<prompt_ner_check>/<prompt_rag>`,
  wiki.py `<prompt_wiki_article>` + JSON-теги). Теги достаются единым
  `get_tagged_prompt` (открывающий тег — только в начале строки:
  упоминания в «#»-комментариях не захватываются). Файл без тегов =
  промпт стадии целиком (легальный режим). Автоподхват — только
  реально существующие файлы: удалённый
  промпт не остаётся выбранным (ни из .env-памяти, ни в форме);
  «Общий промпт-файл» предзаполняется победителем auto-режима
  (`options.auto_prompt`). Кэш опций инвалидируется по max mtime файлов
  папки — перезапись промпта тоже обновляет списки.
- **Перевод с расширенным контекстом** (действие 9, малоресурсные
  языки): словарь/правила/примеры из `source/` через «Файлы»:
  `dict.json` — словарь в формате ner.json (term/translation/type?/
  aliases?/notes?), ищется как глоссарий по чанку + сторонам
  выбранных примеров → `{dict_block}` (JSON);
  `rules.txt|md` — справочник языка, целиком в `{rules_block}`
  (общий потолок — `--request_budget` СИМВОЛЫ);
  `examples.json` — пары `{original_text, translated_text}` (алиасы
  source/target; совместим с `translated_trace.json`), релевантные
  чанку пары — few-shot `{fewshot_block}` (JSON; containment n-грамм
  по ОБЕИМ сторонам пары — автодетект направления перевода, топ-K
  со штрафом за пересечение, порог `fewshot_threshold` — лучше без
  примеров, чем с шумными; словарь — та же автодетект-логика:
  совпадения по term и translation, где больше — та сторона).
  Плейсхолдеры заменяются во всех стадиях (нет файла — «(нет)»);
  промпт перевода — тег `<translate_lr>` (фолбэк `<translate>` →
  встроенный); `--request_budget` — общий бюджет user-запроса СИМВОЛЫ
  (превышение — ошибка чанка); предпросмотр запроса учитывает блоки.
- **Глоссарий — всегда ner.json**: по архитектуре проекта чтение и
  ...
  («Таймаут, сек»); скриптам с раздельными таймаутами
  (`--stream_timeout`) значение передаётся тем же числом.
- **Настройки запусков** (R9): поля форм предзаполняются по слоям
  конфига (`GET /api/stages/{key}/spec?project=…`): `os.environ` →
  системный корневой `.env` → `pdir/.env` (по ключам; пустые значения
  проектного файла не затеняют глобальные; секреты — `api_key` — только
  из `pdir/.env`), при каждом запуске сохраняются в `.env` проекта —
  копия системного корневого `.env` создаётся в папке проекта при
  первом запуске, затем обновляются ключи `<STAGE>_<FIELD>` (напр.
  `NER_CHUNK_SIZE`, `TRANSLATE_CHECK_EXCLUDE_WORDS`; сервер —
  `<STAGE>_HOST` → `HOST`, ключ — `<STAGE>_API_KEY` → `API_KEY`,
  модель — `<STAGE>_MODEL` → общая `MODEL`). LLM-подключение
  (host/model/api_key) пишется только при отличии от глобального
  эффективного значения (совпадение снимает оверрайд — смена сервера
  в compose/system .env доезжает до всех проектов). Ключ сохраняется
  как `<STAGE>_API_KEY` (fallback — `API_KEY`); профили убраны.
- **Экспертный режим — блок «Настройки LLM»**: поля с `group: "llm"
  в спеке (host/api_key/model, temperature, reasoning, max_tokens,
  таймауты, повторы, потоки) группируются в конец формы (только
  порядок отображения; видимость подрежимов и логика запуска не меняются).
...
Стадия `ner_check` — «Режимы» одним select: «Выбранные типы
(одновременно)» (`whole`, по умолчанию) и «Выбранные типы (по очереди)»
(`types`); режим «Полный цикл» убран. Карточки-пресеты убраны
...
`output` (по умолчанию `tmp/translation_quality_assessment.md`): техническая шапка (дата, диапазон, пакет, бюджет, модель) +
оценка LLM. На вкладке «Проверки» секция «Оценка перевода (LLM)»
рендерит md-отчёты из `tmp/` проекта (выбор файла, дефолт
`tmp/translation_quality_assessment.md`).

Стадия `compile`: режимы `txt` («TXT (Rulate)»), `txt-plain` («TXT»),
`epub`, `fb2`; поле «Глав в части» (`chunk_size`) работает для любого
режима: >0 — диапазон разбивается на части по столько глав, пусто/0 —
одна сборка. Все файлы сборки пишутся в `tmp/` проекта с названием
проекта в имени: `{Имя}_{начало}_{конец}_txt.txt` / `.epub` / `.fb2`.
TXT (Rulate)
...
секций (снятие = `skip` с перенумерацией, без перезапуска
предпросмотра),
...
| POST | `/api/stages/batch_replace/preview` | batch_replace: предпросмотр замен в одной главе — `{project, type, chapter, replacements}`; правила — тем же путём, что реальный запуск (`cli.batch_replace.parse_replace_lines`/`apply_rules_segments`); пустые правила — текст главы без изменений; ответ: `segments` `keep/del/ins` итогового текста + `stats` по правилам; файлы не изменяются |
| POST | `/api/stages/{key}/preview-request` (`{project, params}`) | LLM-стадии (pipeline, ner, ner_check, translate_check_llm, translate_quality, wiki — флаг `preview` в спеке): предпросмотр ПЕРВОГО LLM-запроса без сети — синхронный запуск скрипта стадии с `--preview-request` (cwd=проект, артефакты/логи не создаются); ответ — payload `{stage, label, model, messages, chars (СИМВОЛЫ), meta}`; таймаут 300 с, ошибка скрипта — 400 |
