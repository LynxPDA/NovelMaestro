...
`./.env` в compose-файле по умолчанию **закомментирован**: создаёте его
из шаблона (`cp templates/.env.example .env`) и раскомментируете блок
`env_file` — либо задаёте те же переменные в секции `environment`.
Без `.env` контейнер работает со встроенными дефолтами (LLM-профили
вводятся в web-интерфейсе).
...
| `./.env` | env-переменные | создаётся из `templates/.env.example` (`cp`), пробрасывается `env_file` (по умолчанию закомментирован — раскомментируйте); приоритет: env > файл; LLM-скрипты в subprocess наследуют окружение, `get_server_config` читает окружение прежде файла |
