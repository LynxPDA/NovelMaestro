...
`HOST`/`API_KEY`/`MODEL` и `WEB_*` задаются **переменными окружения**
в `docker-compose.yml` (секция `environment` — нужные строки там
закомментированы, раскомментируйте и впишите свои значения). Приоритет:
окружение > `.env`-файл, поэтому environment переопределяет `/app/.env`.
Правки применяются после `docker compose up -d` (пересоздание).
...
| `/app/.env` (в образе) | копия `templates/.env.example` — значения по умолчанию; переопределяются переменными окружения (`environment` в compose) |
