...
конфиг — `.env` (см. `templates/.env.example`; в Docker — `env/.env`), данные —
...
Файл `.env` создаётся **сам** при первом запуске: compose монтирует
папку `./env` в `/app/env.d`, а `docker-entrypoint.sh` копирует туда
`templates/.env.example` (существующий пользовательский `.env` не
затирается). Правьте конфиг в `env/.env` на хосте или во вкладке
«Настройки» web-интерфейса — она пишет в тот же файл (через симлинк
`/app/.env` → `/app/env.d/.env`), правки переживают пересоздание
контейнера и применяются без перезапуска.

Или вручную (без compose):

```bash
mkdir -p env && cp templates/.env.example env/.env   # конфиг
ln -s env/.env .env                                  # симлинк (как в entrypoint)
...
| `./env/.env` | `/app/env.d/.env` (симлинк `/app/.env`) | создаётся сам из `templates/.env.example` при первом старте (entrypoint); вкладка «Настройки» пишет в него; правки переживают пересоздание контейнера и применяются без перезапуска |
...
**Конфиг в Docker**: основной путь — `./env/.env` на хосте (создаётся
сам из шаблона при первом старте; симлинк `/app/.env` →
`/app/env.d/.env`). Вкладка «Настройки» web-интерфейса пишет прямо
в этот файл — правки переживают пересоздание контейнера и применяются
без перезапуска (сервер читает .env заново при каждом запросе).
Править можно и вручную: `env/.env` на хосте, затем обновить вкладку
«Настройки» (или просто перезапустить сервер).
...
- Переживает: `env/.env` (конфиг) — создаётся сам из шаблона, вкладка
  «Настройки» пишет в него; смонтирован через `./env:/app/env.d`.
