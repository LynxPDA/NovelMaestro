...
Конфиг **наружу не пробрасывается** (в образ не входит — заводского
`/app/.env` в образе нет). Реальные
...
`WEB_PROJECTS_DIR` (не нужен при монтировании в дефолтный путь). Только
WEB_* — деплой-слой; LLM-конфиг (HOST/API_KEY/MODEL, *_MODEL) — только
в системном .env (вкладка «Настройки»).

**Конфиг в Docker**: environment в compose — ТОЛЬКО деплой-обвязка
(`WEB_*`, `TZ`) — ключи запуска контейнера. LLM-конфиг и дефолты стадий
(`HOST`/`API_KEY`/`MODEL`, `*_MODEL`, `TRANSLATE_CHECK_*`, `PIPELINE_*`)
задают в **системном .env** — вкладка «Настройки» web-интерфейса или
файл `projects/.env` на хосте (постоянный том; сид из шаблона при
первом старте; переживает обновление образа). Проектные `pdir/.env`
(том projects) перекрывают системный по ключам. Приоритет:
`environment` compose (WEB_*) > `projects/.env` > встроенный дефолт.
...
- Конфиг — environment в compose (WEB_*: запуск контейнера); LLM-конфиг
  и дефолты стадий — только в системном .env (том projects, вкладка
  «Настройки»); при пересоздании контейнера применяются текущие значения.
