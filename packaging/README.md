...
- Контейнер работает от root (bind-mount `./projects` наследует владельца
  хост-папки — non-root ломал бы монтирование на Linux). Для изоляции
  используйте `user:` в compose или запускайте через `--user`.
...
- `pyahocorasick` ставится по возможности (колесо); если нет — сборка
...
1. **LICENSE** — в репозитории его нет: выберите (например, MIT или
   Apache-2.0) и добавьте `LICENSE` до публикации.
...
3. **CI/релизы** — сборки можно выложить в GitHub Releases: Docker —
   `docker buildx build --platform linux/amd64,linux/arm64 -t … --push`,
   портативную папку — zip-артефактом (собирать на Windows runner).
