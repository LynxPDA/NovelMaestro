...
### Конфигурация: системный projects/.env

Весь серверный конфиг — в `projects/.env` (LLM-профили, модели по
...
вводом — не падать. UI-предпочтения (тема, авто-обновление) — в
localStorage браузера, НЕ в .env.
...
8. **Стандарт коммитов** (с раунда 22): `<тип>(<область>): <описание на
   ...
   один коммит. Первый коммит истории: `chore(repo): initial commit —
   NovelMaestro`.
