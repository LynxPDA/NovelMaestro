...
- `cli/wiki.py`: `--as-chapter` — вики как дополнительная последняя
  глава `chapters/00000_{N+1}_Wiki_Новеллы/` (`chapter.txt` +
  `polished.txt`), название «Wiki Новеллы» простым текстом, статьи в
  rulate-стиле (заголовки глубже); компилятор берёт заголовок главы из
