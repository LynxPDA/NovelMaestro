...
### R3-3. epub-предпросмотр: чекбокс без перерисовки страницы

* [x] Чекбокс в списке предпросмотра epub вызывал render() → вся
      страница перерисовывалась; теперь — только classList.toggle
      строки (epub-prev-skip), без перезапуска предпросмотра.
* [x] 943 теста зелёные, node --check чист.

### R3-4. Windows-сборка: robocopy-код 1 убивал шаг

* [x] Лог: скрипт дошёл до «ГОТОВО», архив собран, но шаг упал —
      exit code 1. Причина: robocopy (последняя внешняя команда)
      возвращает 1 при успехе, обёртка powershell в GitHub Actions
      выходит с последним $LASTEXITCODE.
* [x] Фикс: $global:LASTEXITCODE = 0 в конце скрипта.
* [x] Релиз v0.1.0: тег пересоздан, оба воркфлоу, релиз с zip.

### R3-5. TODO.md исключён из сборок + инструкция по релизам

* [x] TODO.md в .dockerignore, robocopy /XF и .gitignore (сам файл
      остаётся в git — это внутренний план разработки).
* [x] packaging/README.md: раздел «Публикация релиза» — тег, проверки,
      пересборка с тем же тегом, черновые прогоны.

### R3-6. Шаблонный replacements.txt удалён

* [x] templates/General/prompts/replacements.txt удалён из репо: с
...
* [x] is_cjk покрывает кану (3040–30FF) и хангыль (AC00–D7AF) —
      японский/корейский обрабатываются тем же кодом.

### R3-9. NER-промпт: универсальное поле произношения

* [x] Поле "pinyin" в ner_prompt.txt заменено на "reading" (оба
      JSON-блока) — ner.py ищет "pinyin", затем "reading"; описание
      гласит: для китайского — пиньинь с тонами; секция «ПРОВЕРКА
