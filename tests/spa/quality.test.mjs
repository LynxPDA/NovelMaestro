...
// из бюджета вычитается длина промпта, ответ — последняя глава,
// помещающаяся в бюджет (не более максимальной).
...
test("все главы помещаются — возвращает последнюю", () => {
  const fn = load();
  assert.equal(fn(TREE, 1, 200000, 0, "polished"), "5");
});

test("обрезка по бюджету до целого количества глав", () => {
  const fn = load();
  // бюджет 7500 = ровно 5 глав по 1500 (на 6-ю не хватает)
  assert.equal(fn(TREE, 1, 7500, 0, "polished"), "5");
  // бюджет 4500 — влезают 3 главы (3×1500)
  assert.equal(fn(TREE, 1, 4500, 0, "polished"), "3");
  // 4501 — та же 3 (четвёртая 1500 не влезает целиком)
  assert.equal(fn(TREE, 1, 4501, 0, "polished"), "3");
});

test("промпт вычитается из бюджета", () => {
  const fn = load();
  // промпт 500 → доступно 4000 → 2 главы (3000)
  assert.equal(fn(TREE, 1, 4500, 500, "polished"), "2");
});

test("промпт больше бюджета — пусто", () => {
  const fn = load();
  assert.equal(fn(TREE, 1, 1000, 2000, "polished"), "");
});

test("бюджета не хватает на первую главу — пусто", () => {
  const fn = load();
  assert.equal(fn(TREE, 1, 1499, 0, "polished"), "");
});

test("start смещает отсчёт", () => {
  const fn = load();
  // с 3-й главы: 3+4+5 = 4500 — все три влезают
  assert.equal(fn(TREE, 3, 4500, 0, "polished"), "5");
  // с 4-й: 4+5 = 3000; бюджет 4500 — обе
  assert.equal(fn(TREE, 4, 4500, 0, "polished"), "5");
  // с 4-й: бюджет 3000 — ровно 4 и 5 (3000) — обе
  assert.equal(fn(TREE, 4, 3000, 0, "polished"), "5");
});
...
test("пропуск главы (нет id 4)", () => {
  ...
  assert.equal(fn(tree, 1, 6000, 0, "polished"), "5");
});

test("start больше последней главы — пусто", () => {
  const fn = load();
  assert.equal(fn(TREE, 6, 200000, 0, "polished"), "");
});
