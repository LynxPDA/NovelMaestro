...
def trim_rule_left(raw: str) -> str:
    """Левая часть правила замены без паддинга вокруг «->».

    «^  » (пробелы после якоря начала строки) и «  $» (пробелы перед
    якорем конца строки) — значимы: это удаление отступов/хвостовых
    пробелов, паддинг их не съедает. Одиночный пробел у якоря — паддинг.
    NFC на выходе; пустая строка — пустой паттерн.
    """
    raw = unicodedata.normalize("NFC", raw or "")
    stripped = raw.strip()
    if not stripped:
        return stripped
    # «^  » — пробелы ПОСЛЕ «^» значимы
    if re.fullmatch(r"\^ {2,}", raw):
        return raw
    # «  $» — пробелы ПЕРЕД «$» значимы (паддинг у стрелки убираем)
    if re.fullmatch(r"^ {2,}\$ *", raw):
        return raw.rstrip()
    return stripped


def trim_rule_right(raw: str) -> str:
    """Правая часть правила замены: паддинг убирается, но правая
    часть из одних пробелов сохраняется («\\s+ -> » — сжать пробелы
    в один). NFC на выходе; пустая строка — удаление совпадений.
    """
    raw = unicodedata.normalize("NFC", raw or "")
    stripped = raw.strip()
    if stripped or not raw:
        return stripped
    return raw  # «\s+ -> » — пробелы в правой части значимы


_RULE_FLAGS_RE = re.compile(r"\s\|([iIrR]+)\s*$")


def strip_rule_flags(line: str) -> tuple[str, str]:
    """Отделяет флаги «|i»/«|r» в конце строки правила от самой строки.

    Разделитель флагов — ровно один пробельный символ перед «|»:
    лишние пробелы остаются в правиле и значимы («\\s+ ->  |r» —
    сжатие пробелов с флагом regexp, а не удаление). Возвращает
    (строка без флагов, флаги в нижнем регистре: «», «i», «r», «ir»…).
    """
    m = _RULE_FLAGS_RE.search(line)
    if not m:
        return line, ""
    return line[:m.start()], m.group(1).lower()
...
def strip_line_comment(line: str) -> str:
    """Отрезает inline-комментарий « # …» в конце строки правила.

    Комментарий — от первого пробела перед «#» до конца строки
    («паттерн -> замена  # пояснение»); «#» без пробела слева
    (например «#[0-9a-f]+» — цвет) комментарием не считается.
    Строка, начинающаяся с «#», — комментарий целиком → ""
    (флаги « |i»/« |r» должны идти ДО комментария; strip_rule_flags
    вызывается ПОСЛЕ этой функции).
    """
