...
Каждая стадия — карточка с формой на странице «Запуски» проекта. Между стадиями — человеческие контрольные точки: правки глоссария и перевода принимает человек, а не машина.
...
Данные живут в папках `projects/` и `templates/` (монтируются в контейнер). Подробности — порты, переменные, обновление — в [packaging/README.md](packaging/README.md).

Параметры сервера: `python3 run.py --no-open` (без браузера), `--projects-dir <путь>` (своя папка проектов), а также `--host/--port/--auth/--token` — см. `python3 web/main.py --help`; подробности настройки сервера — [DEVELOPERS.md](DEVELOPERS.md).
...
Внутри проекта вкладка **«Файлы»** → «＋ Загрузить»: положите `.epub` или `.txt` в папку `source/`.
...
3. **Проверка глоссария (LLM)** — LLM предложит правки; примите их во вкладке «Проверка»;
...
Над каждой карточкой — переключатель **«Простой режим / Экспертный»**: в простом режиме достаточно нажать «Запустить» — параметры возьмутся из настроек; экспертный открывает все поля формы.

Во время запуска — живой лог и прогресс; кнопка **«Стоп»** останавливает работу. История запусков — на «Дашборде» и во вкладке «Логи».

### 4. Правьте тексты

- **Редактор** — главы проекта: оригинал слева, перевод справа, подсветка терминов глоссария;
...
- **Настройки** — внешний вид и интерфейс (хранятся в браузере, localStorage; конфиг сервера/LLM — `.env` на диске, не в UI);
...
Внутри проекта — вкладки: **Файлы, Редактор, Глоссарий, Проверка, Главы, Статус, Конфиг, Промпты, Логи, Заметки** и страница **Запуски**.
